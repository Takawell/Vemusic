var Album = {
    init() {
        ...
        <div id="album-modal" class="fixed inset-0 bg-[#050507] flex flex-col z-30" style="display:none; animation: slideUp 0.3s ease-out forwards;">
            <div class="flex items-center gap-3 p-4 pt-6 bg-transparent absolute top-0 left-0 w-full z-10" id="album-header">
                <button onclick="Album.close()" class="glass glass-hover rounded-full text-white p-3 active:scale-90 shadow-md backdrop-blur-md bg-black/40"><i data-lucide="arrow-left" class="w-6 h-6"></i></button>
            </div>
    ...
    open(id, passedCoverUrl) {
        ...
        .then(data => {
            ...
            <div class="relative w-full aspect-square md:aspect-video max-h-[45vh] overflow-hidden -mt-20">
                <img src="${im}" class="w-full h-full object-cover blur-3xl opacity-50 absolute inset-0 scale-125" />
                ...
                <div class="absolute bottom-6 left-6 right-6 flex items-end gap-6 z-10">
                    <img src="${im}" class="w-32 h-32 md:w-48 md:h-48 rounded-xl shadow-2xl object-cover border border-white/10" onerror="this.src='${FI}'" />
                    <div>
                        <p class="text-[10px] font-bold text-[#cfd3d8] uppercase tracking-[0.2em] mb-1">ALBUM / PLAYLIST</p>
                        <h1 class="text-3xl md:text-5xl font-black text-white mb-2 leading-tight drop-shadow-lg line-clamp-2">${es(a.title)}</h1>
                        ${a.description ? `<p class="text-[#cfd3d8] text-xs md:text-sm line-clamp-2">${es(a.description)}</p>` : ''}
                    </div>
                </div>
            </div>
            ...
            <div class="px-6 mt-6">
                <div class="flex items-center gap-4 mb-6">
                    <button onclick="Album.playAll('${id}')" class="bg-white hover:bg-gray-200 text-black w-14 h-14 rounded-full flex items-center justify-center active:scale-95 transition-all shadow-xl shadow-white/20">
            ...
            if(a.songs && a.songs.length > 0) {
                ...
                a.songs.forEach((s, i) => {
                    ...
                    <div onclick="Album.playSong('${id}', ${i})" class="flex items-center gap-3 p-3 rounded-xl cursor-pointer group active:scale-[0.98] transition-all ${rowBg}">
                        <div class="w-6 text-center text-[#6b7280] text-sm group-hover:text-white shrink-0">${numHtml}</div>
                        ...
                        <div class="text-xs text-[#6b7280] font-mono opacity-80 shrink-0">${es(s.duration)}</div>
                        <button onclick="event.stopPropagation();showPlaylistPicker(S['album_'+'${id}'][${i}])" class="text-[#6b7280] hover:text-white p-2 active:scale-90 opacity-0 group-hover:opacity-100 transition-opacity shrink-0"><i data-lucide="plus" class="w-5 h-5"></i></button>
                    </div>`;
                });
                html += '</div>';
            } else {
    ...
    renderActive() {
        ...
        for (var i = 0; i < songs.length; i++) {
            ...
            if (titleEl) {
                titleEl.className = (isCur ? 'text-rose-400 font-bold' : 'text-white font-medium') + ' text-base truncate mb-0.5';
            }
        }
        lucide.createIcons();
    },
    ...
    playAll(id) {
        ...
        if(pl) {
            pl.songs = S['album_'+id].map(s => {
                const im = s.cover || (s.videoId ? `https://i.ytimg.com/vi/${s.videoId}/hqdefault.jpg` : FI);
                return {
                    id: s.videoId,
                    videoId: s.videoId,
                    title: s.title,
