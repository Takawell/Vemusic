...
const S={ht:[],sr:[],ar:[],sq:'',filter:'all',ct:null,pl:[],pi:-1,ps:'',ip:false,il:false,rm:'all',isShuffle:false,currentAccentColor:'#f43f5e',autoNext:true,iv:null,pt:0,pd:0,at:'home',ld:{type:'none',lines:[]},cli:-1,lo:false,lyricOffset:0,playbackRate:1.0,sleepSecondsLeft:0,sleepEndWithTrack:false};
...
AU.addEventListener('ended',function(){ST();if(handleTrackEnded())return;if(S.rm==='one'){AU.currentTime=0;AU.play().catch(function(){});}else{NX();}});
...
function UB(){
    ...
    if(S.il){
        ...
        if(statusTag){
            statusTag.classList.remove('hidden', 'bg-amber-500/20', 'text-amber-300', 'border-amber-500/30');
            statusTag.classList.add('inline-block', 'bg-blue-500/20', 'text-blue-300', 'border-blue-500/30', 'animate-pulse');
            statusTag.innerText='MEMUAT';
        }
        if(playWrap){
            playWrap.style.boxShadow='0 0 20px rgba(59,130,246,0.5)';
        }
    }
    else if(S.ip){
        ...
        if(statusTag){
            if(S.ct){
                statusTag.classList.remove('hidden', 'bg-blue-500/20', 'text-blue-300', 'border-blue-500/30', 'animate-pulse');
                statusTag.classList.add('inline-block', 'bg-amber-500/20', 'text-amber-300', 'border-amber-500/30');
    ...
    var fullBeats = gid('full-beats-bg');
    if(fullBeats) {
        if(S.ip) {
            fullBeats.querySelectorAll('.full-beat-bar').forEach(function(b){ b.style.animationPlayState = 'running'; });
        } else {
            fullBeats.querySelectorAll('.full-beat-bar').forEach(function(b){ b.style.animationPlayState = 'paused'; });
        }
    }
    if(typeof Search !== 'undefined' && Search.renderActive) {
        Search.renderActive();
    }
    if(typeof Home !== 'undefined' && Home.renderActive && S.at === 'home') {
        Home.renderActive();
    }
    if(typeof Album !== 'undefined' && Album.renderActive) {
        Album.renderActive();
    }
    if(typeof Artist !== 'undefined' && Artist.renderActive) {
        Artist.renderActive();
    }
    if(typeof Library !== 'undefined' && Library.renderActive) {
        Library.renderActive();
    }
    lucide.createIcons();
}
    ...
    try{
        ...
        if(d.status&&d.result.lyrics&&d.result.lyrics.lines.length>0){
            ...
            S.ld.lines.forEach(function(li,i){
                html+='<p class="lyric-line text-left cursor-pointer" data-time="'+li.time+'" onclick="SLT('+li.time+')">'+es(li.text)+'</p>';
            });
            html+='<p class="text-center text-[#4b5563] text-xs mt-8 mb-4 opacity-50 tracking-widest">——— end ———</p>';
            if(c) { c.innerHTML=html; c.classList.remove('hidden'); }
...
function ULH(ct, forceScroll){
    if(!S.ld || !S.ld.lines || S.ld.lines.length===0) return;
    
    var ni=-1;
    for(var i=0; i<S.ld.lines.length; i++){
        if(ct >= S.ld.lines[i].time){ ni=i; }
    }
    ...
    // If lyrics overlay is closed, don't touch DOM or scroll
    if(!S.lo) return;
    ...
    var prevActive = content.querySelector('.lyric-line.active-lyric');
    if(prevActive) prevActive.classList.remove('active-lyric');

    if(ei < 0) return;

    var lines = content.querySelectorAll('.lyric-line');
    var targetLine = lines[ei];
    if(!targetLine) return;

    targetLine.classList.add('active-lyric');
    ...
    if(container && (forceScroll || Date.now() - lastUserLyricScroll > 2500)) {
        ...
        var offset = targetTop - (containerHeight / 2) + (targetHeight / 2);
            ...
            if(p.songs && p.songs.length){
                p.songs.forEach(function(s){
                    if(!s.cover || s.cover.includes('uZKDQkZ3c5VK.png') || s.cover.includes('logo.png')){
                        s.cover=s.videoId?'https://i.ytimg.com/vi/'+s.videoId+'/hqdefault.jpg':FI;
                        changed=true;
                    }
                });
            }
...
function showToast(msg){var toast=document.createElement('div');toast.className='fixed bottom-24 left-1/2 -translate-x-1/2 bg-white text-black font-bold px-5 py-2.5 rounded-full shadow-2xl z-[999]';toast.style.animation='slideUp 0.3s ease-out forwards';toast.innerText=msg;document.body.appendChild(toast);setTimeout(function(){toast.remove();},2000);}
