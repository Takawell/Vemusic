...
app.all('/api/stream', require('./api/stream.js'));
...
app.use((req, res) => {
    ...
    if (req.path.startsWith('/play/')) {
        ...
        if (videoId) {
            ...
            const playTitle = `Dengarkan Musik - NanzMusify`;
            const playDesc = `Dengarkan lagu favoritmu di NanzMusify Web Music Player`;
